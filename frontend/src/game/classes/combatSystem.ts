...
export class CombatSystem {
  ...
  private maxDarklings: number = 5;
  private difficultyScalingTimer: Phaser.Time.TimerEvent | null = null;
  ...
  private startDifficultyScaling(): void {
    this.difficultyScalingTimer = this.scene.time.addEvent({
      delay: 20000,
      callback: this.increaseMaxDarklings,
      callbackScope: this,
      loop: true,
    });
  }

  private increaseMaxDarklings(): void {
    this.maxDarklings += 5;
    console.log(`Increased max darklings to: ${this.maxDarklings}`);
  }
  ...
  private spawnDarkling(): void {
    const maxDarklings = 70;
    if (!this.player || this.darklings.length >= maxDarklings) return;

    const spawnX = this.player.x + Phaser.Math.Between(-50, 50);
    const spawnY = this.player.y - 5;
  ...
  private checkAndSpawnDarklings(): void {
    const missingDarklings = this.maxDarklings - this.darklings.length;
